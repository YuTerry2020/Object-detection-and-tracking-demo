...
def processor(img):
    ...
    if matched.size >0:
        for trk_idx, det_idx in matched:
            ...
            x_box[trk_idx] = xx
            tmp_trk.box =xx
            tmp_trk.class_id = z_box[det_idx][-1]
            tmp_trk.hits += 1
    ...
    for trk in tracker_list:
        if ((trk.hits >= min_detection) and (trk.no_losses <=max_detection)):
             ...
             x_cv2 = trk.box             
             img= tracking_utils.draw_box_label(trk.class_id, trk.id, img, x_cv2)
    # print('the length of trk ', len(tracker_list), ' the length of class', len(z_class))
